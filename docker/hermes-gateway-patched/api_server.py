...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        """Append out-of-band text to an API/dashboard chat's transcript.

        The original stub returned "API server uses HTTP request/response, not
        send()". That is true of a *reply*, which rides the open request, but
        it is not true of delivery in general: a cron job finishing at 04:00
        has no request to ride, and every path that tried — the live adapter,
        the standalone fallback, and `send_message` — resolved to that stub and
        reported the job's own delivery as failed. Any automation created from
        the dashboard stamps `platform: api_server` as its origin, so this made
        the whole surface structurally undeliverable rather than merely
        request-scoped.

        This surface has no socket to push down, but it does have the thing the
        panel actually renders: the session transcript in `state.db`. Appending
        there IS delivery here — the message is present the next time that chat
        is opened. No model turn is involved, which matters because the jobs
        that need this are `no_agent` ones whose entire point is to cost no
        tokens; waking a turn just to carry text already in hand would put the
        spend back on exactly the jobs built to avoid it.

        For this platform the session id IS the chat id (`dash-…` / `api-…`
        rows carry it as `sessions.id`, leaving `chat_id`/`session_key` NULL),
        which is also why `gateway.mirror._find_session_id` — an origin-column
        lookup — finds nothing here and the transcript mirror silently no-ops.
        We resolve by id and deliberately do NOT create a session when it is
        missing: a chat nobody opened is not a destination, and inventing one
        would park briefings in a transcript with no reader. That case is a
        real, reportable failure, so it returns an error rather than a
        success nobody could see.

        Appended as a *user* turn with a provenance prefix, matching the
        deliberate choice in `cron.scheduler._maybe_mirror_cron_delivery`. The
        text is not this agent speaking, and role metadata does not survive the
        SQLite boundary — only role and content persist — so an assistant-role
        append is indistinguishable on replay from a real assistant turn and
        produces the `assistant → assistant` pairs that break strict-alternation
        providers (issue #2221). A user-role turn collapses safely via
        `repair_message_sequence`'s consecutive-user merge, and the prefix keeps
        the next turn in that chat from reading the brief as something the user
        typed.
        """
        text = (content or "").strip()
        if not text:
            return SendResult(success=False, error="refusing to deliver empty content")

        session_id = str(chat_id or "").strip()
        if not session_id:
            return SendResult(success=False, error="no chat_id to deliver to")

        label = (metadata or {}).get("source_label") or "Hermes"
        body = f"[Out-of-band delivery via {label} — not typed by the user]\n{text}"

        def _append() -> Optional[str]:
            """Returns None on success, or a reason the delivery did not land.

            SQLite work is synchronous and this is the gateway's event loop, so
            it runs in a worker thread — a briefing landing must not stall every
            other chat on the box.
            """
            from hermes_state import SessionDB
            ...
            try:
                if db.get_session(session_id) is None:
                    return f"no api_server session {session_id!r} to deliver into"
                db.append_message(session_id=session_id, role="user", content=body)
                return None
            finally:
                db.close()

        try:
            problem = await asyncio.get_running_loop().run_in_executor(None, _append)
        except Exception as e:
            # Never raise into the delivery loop: a raising adapter aborts the
            # remaining targets for the whole job.
            logger.warning("[%s] transcript delivery to %s failed: %s", self.name, session_id, e)
            return SendResult(success=False, error=f"transcript delivery failed: {e}")

        if problem:
            logger.info("[%s] %s", self.name, problem)
            return SendResult(success=False, error=problem)

        logger.info("[%s] delivered into session %s transcript", self.name, session_id)
